...
* và hiển thị chi tiết giỏ hàng tại trang cart2.html
############################################################ */
	...
	if (products[i].incart > 0)
	{
		...
		content += '<tr class="cart_item">';
			content += '<td class="product-remove">';
				content += '<a title="Remove this item" class="remove" onclick="RemoveFromCart('+i+')" href="cart2.html">×</a> ';
			content += '</td>';
			content += '<td>';
				content += '<a onclick="ChangeIdForSingleProduct('+i+')" href="single-product2.html"><img width="145" height="145" alt="poster_1_up" class="shop_thumbnail" src="'+product.thumbnail+'"></a>';
			content += '</td>';
			content += '<td class="product-name">';
				content += '<a onclick="ChangeIdForSingleProduct('+i+')" href="single-product2.html">'+product.name+'</a>';
			content += '</td>';
			...
			content += '<td class="product-quantity">';
				content += '<div class="quantity buttons_added">';
					content += '<a onclick="DecreaseFromCart('+i+')" type="button" class="minus" href="cart2.html">-</a>';
					content += '<input type="number" size="4" class="input-text qty text" title="Qty" value="'+product.incart+'" min="0" step="1">';
					content += '<a onclick="AddToCartIndex('+i+')" type="button" class="minus" href="cart2.html">+</a>';
				content += '</div>';
			content += '</td>';
