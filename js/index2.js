...
* File này thực hiện việc lấy dữ liệu từ localStorage và đổ ra các phần trong trang index2.html
 ############################################################ */
...
for (var i = 0;i<products.length;i++)
{
    content += '<div class="single-product">';
        content += '<div class="product-f-image">';
            ...
            content += '<a onclick="AddToCartIndex('+i+')" href="cart2.html" class="add-to-cart-link"><i class="fa fa-shopping-cart"></i> Thêm vào giỏ hàng</a>';
            content += '<a onclick="ChangeIdForSingleProduct('+i+')" href="single-product2.html" class="view-details-link"><i class="fa fa-link"></i> Chi tiết</a>';
            content += '</div>';
        content += '</div>';
        content += ' <h2><a onclick="ChangeIdForSingleProduct('+i+')" href="single-product2.html">'+products[i].name+'</a></h2>';
...
for (var i = products.length-1;i>= products.length - 3;i--)
{
	...
	content += '<div class="single-wid-product">';
	    content += '<a onclick="ChangeIdForSingleProduct('+i+')" href="single-product2.html"><img src="'+products[i].thumbnail+'" alt="" class="product-thumb"></a>';
	    content += '<h2><a onclick="ChangeIdForSingleProduct('+i+')" href="single-product2.html">'+products[i].name+'</a></h2>';
